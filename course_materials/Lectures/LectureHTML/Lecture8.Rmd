---
title: "Regression Lectures 8-10"
...
.pull-bottom[
```{r, echo = F}
o <- lm_byhand(y = "y", x = c("z", "x"), data = dat, se_type = "HC0", ci=TRUE)
kable(o)
```
]
---
...
```{r, echo = F}
# True value is zero d has no causal effect on y 
yreg <- lm_robust(y~d, data = stacked) %>%
  tidy()%>%
  mutate(model = "y~d")
# d does affect x 
xreg<- lm_robust(x~d, data = stacked)%>%
  tidy()%>%
  mutate(model = "x~d")

# The problem x is post-treatment. 
#Controlling leads to a problem
bcreg <- lm_robust(y ~ d + x, data = stacked)%>%
  tidy()%>%
  mutate(model = "y~d+x")


out <- bind_rows(yreg, xreg, bcreg)%>%
  filter(!grepl("Int|x", term))
